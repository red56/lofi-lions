...
class Heroku < Thor
  ...
  def deploy(target_name, deploy_ref=nil)
    ...
    puts_and_system %{heroku config:add COMMIT_HASH=$(git log -1 --pretty=format:"%h" #{deploy_ref}) -a #{target.heroku_app}}
...
providing the server supports the set_message task. If you give no MESSAGE, it will clear the message"
  ...
  class Sync < Thor
    ...
    def grab
      ...
      capture_cmd = "heroku pg:backups capture #{source.database_url} -a #{source.heroku_app}"
      retrieve_cmd = "curl -o #{source.dump_filename} `heroku pg:backups public-url -a #{source.heroku_app}`"
